...
import { Download, Facebook, Heart, Linkedin, Mail, MapPin, Menu, Phone, X } from "lucide-react";
...
export function SiteShell({ children }: { children: ReactNode }) {
  ...
  const links = [["/", "Accueil"], ["/a-propos", "À Propos"], ["/vie-scolaire", "Vie Scolaire"], ["/contact", "Contact"]] as const;
  return <div className="min-h-screen bg-background">
    <header className="sticky top-0 z-40 border-b border-border/70 bg-background/90 backdrop-blur-xl">
      <div className="mx-auto grid h-20 max-w-7xl grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-4 sm:px-6 lg:grid-cols-3 lg:px-8">
        ...
        <nav className="hidden items-center justify-center gap-6 lg:flex" aria-label="Navigation principale">
        ...
        <div className="hidden items-center justify-end gap-2 lg:flex">
          ...
          <Button asChild variant="outline"><a href="/brochure-angello-school.pdf" download><Download/>Téléchargez la brochure</a></Button>
        </div>
      ...
      {open && <nav className="border-t bg-background px-4 py-5 lg:hidden" aria-label="Navigation mobile"><div className="mx-auto flex max-w-7xl flex-col gap-1">{links.map(([to,label]) => <Link key={to} to={to} onClick={() => setOpen(false)} className="rounded-lg px-3 py-3 font-medium hover:bg-muted">{label}</Link>)}<Button asChild className="mt-2"><Link to="/preinscription" onClick={() => setOpen(false)}>Préinscrire mon enfant</Link></Button><Button asChild variant="outline" className="mt-1"><a href="/brochure-angello-school.pdf" download><Download/>Téléchargez la brochure</a></Button></div></nav>}
    </header>
    <main>{children}</main>
    <footer className="bg-primary text-primary-foreground">
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-14 sm:px-6 md:grid-cols-2 lg:grid-cols-4 lg:px-8">
         <div><div className="flex items-center gap-3"><img src={logoAsset.url} alt="" className="h-16 w-16 rounded-lg bg-background object-contain p-1"/><h2 className="text-xl">ANGELLO School</h2></div><p className="mt-4 text-sm leading-6 text-primary-foreground/75">Une école bilingue qui forme des enfants curieux, confiants et ouverts sur le monde.</p><div className="mt-5 flex gap-3"><Facebook/><Linkedin/><a href={whatsappUrl} aria-label="WhatsApp"><WhatsAppIcon className="h-6 w-6"/></a></div></div>
        <div><h3 className="text-lg">Liens rapides</h3><div className="mt-4 flex flex-col gap-3 text-sm text-primary-foreground/75"><Link to="/a-propos">À Propos</Link><Link to="/vie-scolaire">Vie Scolaire</Link><Link to="/preinscription">Préinscription</Link><Link to="/contact">Contact</Link></div></div>
    ...
    <a href={whatsappUrl} target="_blank" rel="noreferrer" aria-label="Contacter ANGELLO School sur WhatsApp" className="whatsapp-pulse fixed bottom-5 right-5 z-50 grid h-14 w-14 place-items-center rounded-full bg-whatsapp text-primary-foreground shadow-lg"><WhatsAppIcon className="h-8 w-8"/></a>
  </div>;
}
