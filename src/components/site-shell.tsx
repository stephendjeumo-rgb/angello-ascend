...
export function SiteShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background">
    <header className="sticky top-0 z-40 border-b border-border/70 bg-background/90 backdrop-blur-xl">
      ...
      {open && <nav className="border-t bg-background px-4 py-5 lg:hidden" aria-label="Navigation mobile"><div className="mx-auto flex max-w-7xl flex-col gap-1">{links.map(([to,label]) => <Link key={to} to={to} onClick={() => setOpen(false)} className="rounded-lg px-3 py-3 font-medium hover:bg-muted">{label}</Link>)}<Button asChild className="mt-2"><Link to="/preinscription" onClick={() => setOpen(false)}>Préinscrire mon enfant</Link></Button></div></nav>}
    </header>
    <main>{children}</main>
